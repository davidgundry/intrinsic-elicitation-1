...
# of this processing is to make the data entirely anonymous. It also
# culls the data to the first 50 records to follow the
# pre-registration due to unintenional over-collection.
#
...
# using regular expressions. (Originally I did this by hand, hence
# the use of regular expressions, but I quickly decided to re-do the
# same process using a script so my data processing would be
# transparent.)
#
...
filename = "raw/all-data-15-06-2020.json"
...
def reduceToOriginal50(filename):
    game, tool = 0, 0
    output = []
    excluded = []
    with open(filename) as f:
        for line in f:
            x = re.search('"gameVersion":"(Normal|Tool)"', line) 
            if x:
                if "Normal" in x.string:
                    game+=1
                    if game <= 50:
                        output.append(line)
                    else:
                        excluded.append(line)
                elif "Tool" in x.string:
                    tool+=1
                    if tool <= 50:
                        output.append(line)
                    else:
    ...
    outputFile = filename+"+just50"
    with open(outputFile, "w") as f:
        f.writelines(output)
    with open(filename+"+excluded", "w") as f:
        f.writelines(excluded)
    return outputFile
